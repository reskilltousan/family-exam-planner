...
- [x] 4.3 受入基準の自己チェック
  - 確認結果（未実装/改善が必要な項目）：
    - カレンダー週/月ビューとフィルタUIが未実装（現在リスト表示のみ）。
    - メンバー/家族管理のUIなし（familyIdはヘッダー指定のみ、family作成・メンバーCRUD未対応）。
    - タスクは追加のみで、ステータス変更・削除・担当者選択UIなし。
    - イベントメモ/ジャーナル未実装。
    - Google連携のUX不足：OAuth開始導線/連携状態表示/再認可フローなし、手動リロードのみ。
    - バリデーションのUIフィードバック不足（開始/終了逆転や必須入力の表示が不十分）。
    - 認証/家族テナンシのUI側担保なし（APIはfamilyId header依存）。
