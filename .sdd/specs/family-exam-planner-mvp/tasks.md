...
- 学校リスト表示ページ `/highschools` を追加（都道府県＋中高切替でAPI取得、学校名と公式/詳細リンク、お気に入りローカル保存、入試日程メモをローカル保存）。
- トップに「受験先登録」導線を追加し、タブ切替の受験先ページ `/destinations` を作成（中学/高校/大学/大学院）。
- 週次PDF出力ページ `/print-week` を追加（今週/前後週の予定とタスクを印刷・PDF保存可）。トップに「受験先登録」ボタンを設置。
- イベント/タスクの詳細モーダルに添付リンク（Drive等URL）追加機能を実装し、添付一覧をトップに表示。添付はローカル保存。

## 再起動後にやること
- 添付の本格実装：Google Drive API でアップロード＋共有リンク取得（OAuthスコープ、バックエンド経由のプリサイン/アップロード、メタデータ保存）。
- 高校リスト拡充：協会掲載順で神奈川・茨城・栃木・群馬…のパーサ/APIを追加。
- 添付の保存先をサーバー側に昇格するか検討（認証/テナンシ含む）。
